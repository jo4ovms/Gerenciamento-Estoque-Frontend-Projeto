...
import ImportExportButtonsFornecedor from '../dashboard/components/ImportExportButtonsFornecedor';
...
const FornecedorPage = () => {
  ...
  const handleFilterTipoProdutoChange = (e) => setFilterTipoProduto(e.target.value);
  ...
  return (
    <PageContainer title="Fornecedores" description="this is Fornecedores page">
      <DashboardCard title="Fornecedores">
        <Box display="flex" justifyContent="space-between" alignItems="center">
          ...
          <Button variant="contained" color="primary" onClick={handleClickOpen} sx={{ marginBottom: 2, marginRight: '-400px' }}>
            Criar Fornecedor
          </Button>
          <Box sx={{ display: 'flex', justifyContent: 'center', position: 'relative', top: '-8px' }}>
            <ImportExportButtonsFornecedor iconSize={35} iconStyle={{ color: 'green' }} containerStyle={{ display: 'flex', justifyContent: 'center' }} />
          </Box>
        </Box>
        <Box mt={2}>
          {filteredFornecedores.length > 0 ? (
                ...
                {visibleProducts[fornecedor.id] && (
                  <Box mt={2} pl={2} pr={2} pb={2}>
                    {produtos.length > 0 ? (
                      ...
                      <Typography variant="h6">Nenhum produto registrado ainda.</Typography>
                    )}
                  </Box>
                )}
              </Box>
            ...
            <Typography variant="h7">Nenhum fornecedor registrado ainda.</Typography>
          )}
        </Box>
      </DashboardCard>
      ...
      <Dialog open={open} onClose={handleClose}>
        ...
        <DialogContent>
          <TextField autoFocus margin="dense" name="nome" label="Nome" type="text" fullWidth value={currentFornecedor.nome} onChange={handleChange} />
          <TextField margin="dense" name="email" label="Email" type="email" fullWidth value={currentFornecedor.email} onChange={handleChange} />
          <TextField margin="dense" name="cnpj" label="CNPJ" type="text" fullWidth value={currentFornecedor.cnpj} onChange={handleChange} />
          <TextField margin="dense" name="telefone" label="Telefone" type="text" fullWidth value={currentFornecedor.telefone} onChange={handleChange} />
          <TextField margin="dense" name="tipoProduto" label="Tipo de Produto" type="text" fullWidth value={currentFornecedor.tipoProduto} onChange={handleChange} />
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={openProduto} onClose={handleCloseProduto}>
        ...
        <DialogContent>
          <TextField autoFocus margin="dense" name="nome" label="Nome" type="text" fullWidth value={currentProduto.nome} onChange={handleChangeProduto} />
          <TextField margin="dense" name="valor" label="Valor" type="number" fullWidth value={currentProduto.valor} onChange={handleChangeProduto} />
          <TextField margin="dense" name="quantidade" label="Quantidade" type="number" fullWidth value={currentProduto.quantidade} onChange={handleChangeProduto} />
        </DialogContent>
        <DialogActions>
